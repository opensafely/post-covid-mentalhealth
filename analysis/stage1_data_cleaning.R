...
stage1 <- function(cohort_name) {
  ...
  ### Variable: sub_cat_covid19_hospital ---------------------------------------
  ...
  input$rule1 <- NA
  input$rule1 <- ((input$qa_num_birth_year > (format(input$death_date, format="%Y")) & 
  ...
  ### Rule 2: Year of birth is before 1793 or year of birth exceeds current date
  input$rule2 <- NA
  input$rule2 <- ((input$qa_num_birth_year < 1793 | 
  ...
  input$rule3 <- NA
  input$rule3 <- ((input$death_date <= as.Date(study_dates$earliest_expec) | 
  ...
  input$rule6 <- NA
  input$rule6 <- (input$qa_bin_prostate_cancer == TRUE & 
                    input$cov_cat_sex=="Female")
  
  ## Keep individuals that meet all QA criteria --------------------------------
  ...
  QA_summary[nrow(QA_summary)+1,] <- c("Rule 2: Year of birth predates 1793 or year of birth exceeds current date",
  ...
  write.csv(QA_summary, 
            file = paste0("output/QA_summary_",cohort_name,".csv"), 
            row.names=F)
  
  # Remove QA variables from dataset -------------------------------------------
  
  input <- input_QA[, !names(input_QA) %in% c("qa_num_birth_year",
                                              "qa_bin_pregnancy",
                                              "qa_bin_prostate_cancer")]
  
  print(paste0(cohort_name,  " cohort: ", nrow(input), " rows in the input file after QA"))
  
  ## Define dataset for cohort flow -------------------------------------------
  ...
  ## Apply common inclusion/exclusion criteria ---------------------------------
  ...
  ### Inclusion criteria 5: Registered in an English GP with TPP software ------
  ...
  ## Check if factor reference levels have changed -----------------------------
  ...
  ## Apply cohort specific inclusion/exclusion criteria ------------------------
  ...
  print(paste0(cohort_name, " cohort: " , nrow(input), " rows in the input file after cohort specific inclusion criteria"))
  
  print(paste0(cohort_name, " cohort: ", nrow(input), " rows in the input file after common inclusion criteria"))
  
  ## Save meta data factors ----------------------------------------------------
  ...
  write.csv(cohort_flow, 
            file = paste0("output/Cohort_flow_",cohort_name, ".csv"), 
            row.names=F)
  ...
  svglite::svglite(file = paste0("output/numeric_histograms_", cohort_name, "_", ".svg"),
                   width = 15, height = 8)
  ...
  ## Save final stage 1 dataset ------------------------------------------------
  
  saveRDS(input, 
          file = paste0("output/input_",cohort_name, "_stage1.rds"), 
          compress = TRUE)
  
}

# Run Stage 1 function for specified cohort ------------------------------------
